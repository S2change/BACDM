...
1. Update `AAA_Configs.py`, namely
  ...
  ```
2. Execute in QGIS `qgis_read_reference_data_set.py`. Set `CLEAR=True` to clear the current QGIS project. The legend for the burned area indicates the month and the labels indicate the day. If available, add to the map the vectorized CCD raw results results for the same ROI and time period of the reference data. Use same legend for both reference and vectorized CCD datasets.
