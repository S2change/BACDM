...
To test locally the results of BADCM over a collection of 16-band chips, and for some reference data set (e.g. burned area product from ICNF), one can perform the following set of steps:
...
9. In QGIS, execute again  `qgis_read_reference_data_set.py`. Set `CLEAR=False` to add the ICNF burned area layer to the existing layers `before`, `after`, and `predict`.
